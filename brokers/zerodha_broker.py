...
class ZerodhaBroker:
    ...
    def __init__(self, broker_user_id, api_key, access_token=None):
    ...
    def _get_access_token(self) -> str:
        if self.session_manager and self.connection_id:
            token = self.session_manager.get_access_token(
                "zerodha", connection_id=self.connection_id
            )
            if not token:
    ...
    def login(self):
        logging.debug(f"Logging in to Zerodha for user {self.broker_user_id}")
        try:
            kite = self._ensure_kite()
            profile = kite.profile()
            logging.debug(
                f"Successfully logged in as {profile.get('user_name', 'unknown')}"
            )
    ...
    def logout(self):
        logging.debug(f"Logging out from Zerodha for user {self.broker_user_id}")
        try:
            kite = self._ensure_kite()
    ...
    def get_gtt_orders(self):
        ...
        try:
            kite = self._ensure_kite()
            logging.info(f"Fetching GTTs from Zerodha for user {self.broker_user_id}")
            gtts = kite.get_gtts()
            ...
            for g in gtts:
                ...
                logging.info(
                    f"Processing GTT: trigger_id={trigger_id}, status={g.get('status')}"
                )
                normalized.append(
                    {
                        "id": trigger_id,
    ...
    def place_gtt(self, **kwargs):
        ...
        try:
            ...
            logging.debug(
                f"GTT params - last_price: {last_price}, trigger_price: {trigger_price}, quantity: {quantity}, price: {price}"
            )
    ...
    def download_historical_trades(self, start_date, end_date):
        logging.info(
            "Downloading historical trades is not implemented for Zerodha yet."
        )
        return []

    def get_historical_data(self, symbol, interval, start_date, end_date):
        logging.warning(
            "get_historical_data is not supported directly by ZerodhaBroker."
        )
        return None
