...
c = s.connect(os.path.join(ROOT, "data", "tradecraftx.db"))
rows = c.execute(
    "SELECT COUNT(*) FROM user_trades WHERE broker=?", ["upstox"]
).fetchone()
...
fn = os.path.join(ROOT, "data", "order_history_upstox_all.csv")
