...
def main():
    ...
    parser.add_argument(
        "--broker", "-b", required=True, help="Broker name (e.g., upstox, zerodha)"
    )
    parser.add_argument("--symbol", "-s", help="Filter by symbol")
    parser.add_argument(
        "--limit", "-l", type=int, default=50, help="Number of trades to show"
    )
    ...
    if args.export:
        filename = f"order_history_{args.broker}_{args.symbol or 'all'}.csv"
        ...
        for t in trades:
            print(
                f"  {t.trade_date} | {t.symbol:10} | {t.side:4} | {t.quantity:4} @ {t.price:8} | ID:{t.trade_id}"
            )
