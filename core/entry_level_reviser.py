...
class EntryLevelReviser:
    ...
    def _fetch_data(self):
        """Fetches historical data for the symbol from OHLCV store."""
        try:
            ...
            if not original_scrip:
                raise ValueError(
                    f"Could not find entry level data for {self.symbol} to determine exchange."
                )

            # Fetch more data to ensure indicators are stable
            try:
                self.historical_data = get_ohlcv(self.symbol, days=200)
            except OhlcvStoreError as e:
                raise ValueError(
                    f"OHLCV data not available. Admin may need to run OHLCV refresh. Error: {e}"
                )
            ...
            # OHLCV store returns dicts with keys: trade_date, open, high, low, close, volume
            self.df.columns = ["trade_date", "open", "high", "low", "close", "volume"]
    ...
    def _calculate_indicators(self):
        ...
        self.metrics["atr_pct"] = (
            (self.metrics["atr"] / self.metrics["ltp"] * 100)
            if self.metrics["ltp"] > 0
            else 0
        )
        ...
        plus_di = 100 * (
            plus_dm.ewm(alpha=1 / self.config["ADX_PERIOD"], adjust=False).mean()
            / tr_adx
        )
        minus_di = 100 * (
            abs(minus_dm.ewm(alpha=1 / self.config["ADX_PERIOD"], adjust=False).mean())
            / tr_adx
        )
    ...
    def _get_new_levels(self):
        ...
        logic = self.config["REVISION_LOGIC"].get(
            regime, self.config["REVISION_LOGIC"]["DEFAULT"]
        )
