...
class RiskManager:
    ...
    def _calculate_atr(self, historical_data: List[Dict]) -> float:
        ...
        # Handle both dict format (from OHLCV store) and list format (legacy from Upstox API)
    ...
    def assess_risk_and_get_adjustments(self, scrip_data: Dict) -> Dict:
        ...
        levels_crossed = sum(
            1 for p in levels if p is not None and not isinstance(p, str) and ltp <= p
        )
        session_drop_pct = (prev_close - ltp) / prev_close if prev_close > 0 else 0.0

        sdt_triggered = (
            levels_crossed >= self.config["SDT_LEVEL_THRESHOLD"]
            or session_drop_pct >= self.config["SDT_DROP_PCT_THRESHOLD"]
            or atr_pct >= self.config["SDT_ATR_PCT_THRESHOLD"]
        )

        if sdt_triggered:
            if levels_crossed == 2:
                adjustments["sdt_cap_pct"] = self.config["SDT_CAPS"]["LEVEL_2"]
            elif levels_crossed >= 3:
                adjustments["sdt_cap_pct"] = self.config["SDT_CAPS"]["LEVEL_3"]
            adjustments["reasons"].append(
                f"SDT: LvlX:{levels_crossed}, Drop:{session_drop_pct:.1%}, ATR%:{atr_pct:.1%}"
            )
