...
class AuthService:
    ...
    def login(
        ...
        *,
        ...
        with self._session_factory() as session:
            ...
            if user.role == "admin" and not user.trading_enabled:
            ...
            return [
                {
                    ...
                    "token_updated_at": (
                        c.token_updated_at.isoformat() if c.token_updated_at else None
                    ),
                }
    ...
    def ensure_dev_user(self) -> UserContext:
        with self._session_factory() as session:
            user = (
                session.query(models.User)
                .filter(models.User.email == "dev@tradecraftx.local")
                .first()
            )
            if not user:
                user = models.User(
    ...
    def set_user_trading_enabled(
        self, user_id: int, enabled: bool, admin_user_id: int
    ) -> models.User:
