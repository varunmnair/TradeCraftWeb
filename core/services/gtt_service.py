...
class GTTService:
    ...
    def place_orders(
        self, session_id: str, plan: List[Dict[str, Any]], *, dry_run: bool = False
    ) -> Dict[str, Any]:
        manager = self._get_manager(session_id)
    ...
    def adjust_orders(
        self, session_id: str, orders: List[Dict[str, Any]], target_variance: float
    ) -> Dict[str, Any]:
        manager = self._get_manager(session_id)
        strategy = self._build_strategy(session_id)
        modified = manager.adjust_orders(
            orders, target_variance, strategy.adjust_trigger_and_order_price
        )
    ...
    def delete_orders_by_ids(
        self, session_id: str, gtt_ids: List[str]
    ) -> Dict[str, Any]:
        manager = self._get_manager(session_id)
    ...
    def adjust_orders_by_ids(
        self, session_id: str, gtt_ids: List[str], target_variance: float
    ) -> Dict[str, Any]:
        manager = self._get_manager(session_id)
        strategy = self._build_strategy(session_id)
        ...
        orders_to_adjust = [
            o for o in all_orders if str(o.get("GTT ID")) in ids_to_adjust
        ]
                    ...
                    failed.append(
                        {
                            ...
                            "Symbol": order["Symbol"],
                            "status": "failed",
                            "reason": "CMP not available - cannot adjust",
                        }
                    )
                    continue
                    ...
                    failed.append(
                        {
                            ...
                            "Symbol": order["Symbol"],
                            "status": "failed",
                            "reason": "Variance not available - cannot adjust",
                        }
                    )
                    continue
                    ...
                    adjusted.append(
                        {
                            ...
                            "Symbol": order["Symbol"],
                            "status": "skipped",
                            "reason": f"Variance {variance}% already >= target {target_variance}%",
                        }
                    )
